...
never been run in production. Those three facts, plus a disaster-recovery
exercise CI has no credential to run, are the substance of the 7-point shortfall
in `MATURITY_REPORT.md`, and they are the honest headline.**
...
* **The published score was not the pipeline's score.** The `evidence` job ran
  with only a database, so the accessibility and performance controls had no
  report to read and scored NOT_EVIDENCED — CI computed 89.19 while this
  repository published 94.59, a figure only a fully-equipped developer machine
  produced. The release gate's floor of 70 meant nothing ever flagged the
  divergence. The job now builds and serves the console, audits it, drives the
  load sweep and requires its services, and the documents quote what it
  computes. The residual gap is DRP-001 alone, named rather than averaged away.
