...
def test_all_migrations_are_applied(db: Session) -> None:
    applied = {r[0] for r in db.execute(text("SELECT version FROM schema_migrations")).all()}
...
def test_modified_migration_is_rejected(tmp_path: Path) -> None:
    ...
    try:
        ...
        with get_owner_engine().begin() as conn:
            conn.execute(sql("DELETE FROM schema_migrations WHERE version = '9001_probe'"))
...
def test_checksums_are_recorded_for_every_applied_migration(db: Session) -> None:
    rows = db.execute(text("SELECT version, checksum FROM schema_migrations ORDER BY version")).all()
    assert rows
    for row in rows:
