...
def probe(provisioning_db, seeded):
    ...
    decision = provisioning_db.execute(
        text(
...
def _refused(session, sql: str, params: dict) -> None:
...
# ------------------------------------------------------------------- isolation
def test_every_new_table_forces_row_level_security(provisioning_db, seeded) -> None:
...
def test_a_second_tenant_cannot_see_another_tenants_decisions(db, db_other, tenant_id, seeded) -> None:
    ...
    seen = db_other.execute(
        text("SELECT count(*) FROM decisions WHERE reference = :r"), {"r": reference}
    ).scalar_one()
    assert seen == 0, "a decision leaked across the tenant boundary"
...
def test_the_transition_log_refuses_truncate(provisioning_db, probe) -> None:
    """A row trigger alone would miss this; 0009 found the same hole in the ledger."""
    _refused(provisioning_db, "TRUNCATE decision_transitions", {})
...
def test_a_confidence_with_its_inputs_is_accepted(provisioning_db, probe) -> None:
    provisioning_db.execute(
        text(
            "INSERT INTO recommendations (tenant_id, decision_id, confidence, confidence_calculation) "
...
def test_a_recommendation_may_carry_no_confidence_at_all(provisioning_db, probe) -> None:
...
def test_a_verdict_cannot_be_claimed_without_verification(provisioning_db, probe) -> None:
    _refused(
        provisioning_db,
...
def test_a_verdict_with_a_verifier_and_a_method_is_accepted(provisioning_db, probe) -> None:
    provisioning_db.execute(
        text(
...
# ---------------------------------------------------------------- other guards
def test_a_decision_must_belong_to_a_domain(provisioning_db, probe) -> None:
...
def test_a_kpi_value_must_have_a_definition(provisioning_db, probe) -> None:
...
def test_an_option_score_outside_zero_to_one_is_refused(provisioning_db, probe) -> None:
    _refused(
        provisioning_db,
